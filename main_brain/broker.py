...
from .paths import BrainPaths, resolve_private_path
...
def _proposal_path(paths: BrainPaths, proposal_id: str) -> Path:
    return paths.audit / "proposals" / f"{proposal_id}.json"


def _affected(paths: BrainPaths, specification: dict[str, Any]) -> list[Path]:
...
def _exact_preview(paths: BrainPaths, specification: dict[str, Any]) -> str:
    operation = specification["operation"]
    if operation in {"edit", "migration"}:
        chunks: list[str] = []
...
def propose(paths: BrainPaths, specification: dict[str, Any]) -> dict[str, Any]:
...
def load_proposal(paths: BrainPaths, proposal_id: str) -> dict[str, Any]:
    return json.loads(_proposal_path(paths, proposal_id).read_text(encoding="utf-8"))


def approve_intent(
    paths: BrainPaths, proposal_id: str, confirmation: str
) -> dict[str, Any]:
...
def _apply(paths: BrainPaths, specification: dict[str, Any]) -> list[str]:
    operation = specification["operation"]
    changed: list[str] = []
    if operation in {"edit", "migration"}:
        for item in specification["changes"]:
...
def approve_exact(
    paths: BrainPaths, proposal_id: str, confirmation: str, agent: str = "unknown"
) -> dict[str, Any]:
    proposal = load_proposal(paths, proposal_id)
