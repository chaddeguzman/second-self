...
def test_legacy_lists_unparseable_note(second_self: SecondSelfPaths) -> None:
    ...
    assert any(item["path"] == "01 Capture/02 Notes/Malformed.md" for item in legacy)
...
def test_legacy_lists_oversized_note(second_self: SecondSelfPaths) -> None:
    ...
    legacy = legacy_items(second_self)
    assert any(item["path"] == "01 Capture/02 Notes/Oversized.md" and item["reason"] == "oversized" for item in legacy)


def test_legacy_ignores_valid_note(second_self: SecondSelfPaths) -> None:
    _note(
        second_self.layer1 / "01 Capture/02 Notes/Valid.md",
        "type: note\ncreated: 2026-07-01\nstatus: active",
        "Valid",
    )
    legacy = legacy_items(second_self)
...
def test_legacy_respects_scope_filter(second_self: SecondSelfPaths) -> None:
    layer1_legacy = second_self.layer1 / "01 Capture/02 Notes/LegacyNote.md"
    layer1_legacy.parent.mkdir(parents=True, exist_ok=True)
    ...
    assert "01 Capture/02 Notes/LegacyNote.md" in layer1_paths
