...
LOCAL_PATHS = [
    ".agents/skills/second-self/",
    ".codex/hooks.json",
    ".codex/second-self-hook.py",
    "CLAUDE.local.md",
]
...
def registration_preview(paths: SecondSelfPaths, project_path: Path, name: str) -> list[str]:
    project_path = project_path.resolve()
    return [
        f"Create private project record: {paths.projects / (_slug(name) + '.md')}",
        f"Create local Codex skill: {project_path / '.agents/skills/second-self/SKILL.md'}",
...
def register_project(
    paths: SecondSelfPaths, project_path: Path, name: str, repository: str = ""
) -> list[Path]:
    project_path = project_path.resolve()
...
""",
    ...
    skill = project_path / ".agents" / "skills" / "second-self" / "SKILL.md"
    skill.parent.mkdir(parents=True, exist_ok=True)
    skill.write_text(
        f"""---
name: second-self
description: Use the private Second Self for personal context, project decisions, recall, and project writeback in this registered local repository.
---

# Second Self

Read `{paths.repo_root / 'AGENTS.md'}` before using Second Self context.
...
protected changes through the Second Self edit broker.
""",
        encoding="utf-8",
    )
    ...
    claude.write_text(
        f"""# Second Self Local Adapter

Read `{paths.repo_root / 'AGENTS.md'}` and use `{record}` as this project's
private Second Self record. Full private context at `{paths.data_root}` is available
for relevant local searches. Protected edits require the Second Self broker.
""",
        encoding="utf-8",
    )
    ...
    local_hook = project_path / ".codex" / "second-self-hook.py"
    ...
    hooks_json.write_text(
        """{
  "description": "Second Self protected-edit policy.",
  "hooks": {
    "PreToolUse": [{
      "matcher": "Bash|apply_patch|Edit|Write",
      "hooks": [{
        "type": "command",
        "command": "python .codex/second-self-hook.py",
        "commandWindows": "python .codex/second-self-hook.py",
        "timeout": 10,
        "statusMessage": "Checking Second Self edit policy"
      }]
    }]
  }
}
    ...
    if additions:
        ...
        with exclude.open("a", encoding="utf-8") as stream:
            if existing and not existing.endswith("\n"):
                stream.write("\n")
            stream.write("# Second Self local adapters\n")
            stream.write("\n".join(additions) + "\n")
    return created
